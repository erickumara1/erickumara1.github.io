...
int bitXor(int x, int y) {
    // Xor(0,0) or Xor(1,1) = 0
    // Xor(1,0) or Xor(0,1) = 1
    int ZeroIfBothOne = ~(x & y);
    int ZeroIfBothZero = ~((~x)&(~y));
    int result = ZeroIfBothOne & ZeroIfBothZero;
    return result;
}
/* 
 ...
 */
int isTmax(int x) {
    //x max is when 011..111
    //2x+2 is 111..110 + 000..010 which is zero 
    // if x is 111..111 then ~x is 000..000, !~x is then 1, !!~x is then 0
    // if x is 011..111 then ~x is 100..000, !~x is then 0, !!~x is then 1
    //important case is when x is 011..111
    int pos_or_neg = !!(~x);
    return (x + x + 1 + pos_or_neg) == 0; 
}
/* 
...
int allOddBits(int x) {
    ...
    //all odds in a 32 bit if 01010101.......
    // if x = 0, x + 170 << 8 looped 4 times is 10101010.....
    int y = 0;
    int even_alter = 170;
    y = y + 170; //___p
    y = y << 8; //__p_
