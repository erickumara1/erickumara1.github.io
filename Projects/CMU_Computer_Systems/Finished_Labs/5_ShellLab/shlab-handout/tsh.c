...
void do_bgfg(char **argv) {
    if(argv[1] == 0){
        printf("missing argument for job or pid");
        return;
    }
    ...
    if(num<0){
        printf("invalid pid or job arugment");
        return;
    }
    if(argv[2]!=0){
...
void waitfg(pid_t pid) {
    ...
    while(1){
        ...
        //job no longer exists or job is no longer in foreground
        if(fg_jid == 0 || (fg_state!=-1 && fg_state!=FG)){
            return;
        }
        sigsuspend(NULL);
    }
